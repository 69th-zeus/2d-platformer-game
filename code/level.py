...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage, change_music, text):
    ...
    def item_collision(self):
        if self.item_sprites:
            item_sprites = pygame.sprite.spritecollide(self.player, self.item_sprites, True) # True to destroy the item player has collided with
            if item_sprites:
    ...
    def run(self, dt):
        ...
        if self.data.health <= 0:
            ...
            self.alpha += 300 * dt
            self.display_surface.blit(self.trans, self.trans_rect)
            self.text("GAME OVER", (640, 300), WHITE, "GO")
            self.text("Window Closing", (640, 430), WHITE, "WC")
            # self.text("SYSTEM EXITING", )
            if self.alpha >= TRANSITION_TIME:
                pygame.quit()
                sys.exit()